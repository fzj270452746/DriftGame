...
let Brie = Hzxuts[0]              //jsBridge
let amt = Hzxuts[1]     //amount
let ren = Hzxuts[2]      //currency
let OpWin = Hzxuts[3]      //openWindow
...
internal class LoxisnFstVC: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {

    var lxosn: Kxoieus?
    var tcuans: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        if lxosn!.viemse != nil {
            view.backgroundColor = UIColor.init(hexString: lxosn!.viemse!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = lxosn!.trizixn!
        AppsFlyerLib.shared().appleAppID = lxosn!.lcpman!
...
//        
        ...
        Hzxuts = lxosn!.diaous!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: lxosn!.cyaisb!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        tcuans = WKWebView(frame: .zero, configuration: cofg)
        tcuans!.allowsBackForwardNavigationGestures = true
        tcuans?.uiDelegate = self
        tcuans?.navigationDelegate = self
        view.addSubview(tcuans!)
        
        kaoieus = lxosn!.cnuac!
        if kaoieus?.contains("://") == true {
            tcuans?.load(URLRequest(url:URL(string: kaoieus!)!))
        } else {
            tcuans?.load(URLRequest(url:URL(string: ncjiays(kaoieus!)!)!))
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = lxosn!.pcuack!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = lxosn!.pcuack!.contains("I") ? view.safeAreaInsets.bottom : 0
            tcuans?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
